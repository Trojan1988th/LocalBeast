...
Providers (TTS_PROVIDER env):
  vibevoice — Microsoft VibeVoice 1.5B, local subprocess, your configured reference voice (worked ~80% tool-call rate)
...
def tts_generate_voice_message(text: str, voice: str | None = None) -> str:
    ...
    VibeVoice: single voice (your VIBEVOICE_REFERENCE_VOICE). Kitten: Bella, Jasper, Luna, Bruno, Rosie, Hugo, Kiki, Leo
...
def _generate_kitten(text_clean: str, voice: str | None) -> str:
    ...
    try:
        ...
        out_path = _TTS_OUTPUT_DIR / f"voice_{stamp}.wav"
